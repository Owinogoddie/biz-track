...
import { updateFundingSource } from '@/app/actions/funding-source'
import { Loader2 } from "lucide-react"
import { FundingSource, FundingType } from '@prisma/client'
...
export function EditFundingSourceModal({ fundingSource, onClose }: EditFundingSourceModalProps) {
  const [isLoading, setIsLoading] = useState(false)
  const { toast } = useToast()
  const { updateFundingSource: updateFundingSourceStore } = useFundingSourceStore()
  ...
  const form = useForm<FundingSourceFormValues>({
    ...
    defaultValues: {
      ...
      startDate: fundingSource.startDate,
      endDate: fundingSource.endDate || undefined,
    },
  })

  async function onSubmit(data: FundingSourceFormValues) {
    setIsLoading(true)
    const result = await updateFundingSource(fundingSource.id, {
      ...data,
      businessId: fundingSource.businessId,
    })
    
    if (result.success) {
      updateFundingSourceStore(result.fundingSource)
      toast({
        title: 'Funding source updated!',
        description: 'Your funding source has been updated successfully.',
      })
      onClose?.()
    } else {
      toast({
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Make changes to your funding source here.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
